...
- Growing rapidly:
	- *Need to scale*
	- *Autoscaling Groups*
	- *Kubernetes autoscaling (pods, [node pools](GKE%20Cluster%20Autoscaling.md))*
- DR plan required
...
- **Web-based customer-facing** applications
	- Many already **containerized** to run on **Kubernetes**:
		...
		- *[GKE Enterprise](GKE%20Enterprise.md) for hybrid-cloud use cases, and to manage multiple clusters centrally*
		- *Google Container Registry*
- **SQL** and **noSQL** DBs:
	- MySQL
		- *Lift/shift to [Cloud SQL](Cloud%20SQL.md)*
	- MS SQL Server
		- *Lift/shift to Cloud SQL*
		- *Provision specific version in [Compute Engine](Compute%20Engine.md) (if not available in Cloud SQL)*
	- Redis
		- *Lift/shift to [Memorystore](Memorystore.md)*
		- *Deploy Redis Enterprise from marketplace*
	- MongoDB
		- *ETL to [Cloud Firestore](Cloud%20Firestore.md)*
...
- Monitoring:
	...
	- Alerts via **email** often **ignored**
		- *Reduce frequency*
		- *Move from email to SMS, Slack, PagerDuty, [Pub/Sub](Pub%20Sub.md) etc.*
		- *Alerting policies*
...
- Onboard **new** insurance providers **as quickly as possible**:
	...
	- *SaaS admin portal*
		- *Deploy to [App Engine](App%20Engine.md) or [Cloud Run](Cloud%20Run.md) (preferred due to containers) if traffic low*
...
- **Reduce latency** to end users:
	...
	- *Multi-zonal / regional resources*
	- *Anthos to place clusters in multiple-regions—[Multi Cluster Ingress](Multi%20Cluster%20Ingress.md)*
...
- **Predictions** and reports on industry trends:
	- *AI/[ML](Machine%20Learning.md)*
	- *[BigQuery ML](BigQuery%20ML.md)*
	- *Reports with [Looker Studio](Google%20Looker%20Studio.md)*
	- *AI Platform / [Vertex AI](moc/Vertex%20AI.md)*
...
- **Outages** caused by:
	- Misconfigurations
		...
		- *Infrastructure as code e.g. [Terraform](moc/Terraform.md)*
		- *[Kubernetes](moc/Kubernetes.md)—roll-back unstable releases*
		- *Anthos Config Management for guardrails and best practice policies across all clusters*
	- Inadequate capacity
		- *[Autoscaling](Google%20Cloud%20Compute%20Autoscaling.md)*

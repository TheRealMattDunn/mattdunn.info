...
- Instance runs with [Service Account](Cloud%20IAM.md)—represents the instance's identity and roles
...
## Deploying to [Compute Engine](Compute%20Engine.md)

- Startup script
	- Supported by both Linux and Windows
	- Define in metadata
	- Or store in [Cloud Storage](Cloud%20Storage.md) and add metadata key `startup-script-url` with the value of the GCS URL for the script
...
- Two options:
	...
	2. Use a Managed Instance Group fronted by a [load balancer](Google%20Cloud%20Load%20Balancing.md) configured with a static DNS name

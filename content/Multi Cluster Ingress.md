...
Google hosted service to load balance requests across [Kubernetes Engine](Kubernetes%20Engine%20(GKE).md) clusters and regions. The MultiClusterIngress [Kubernetes](moc/Kubernetes.md) resource is a controller for [External HTTP(S) Load Balancers](Google%20Cloud%20Load%20Balancing.md).
...
- Multi Cluster Ingress Controller
	- Google managed
	- Programs external HTTP(S) Load Balancer using [Network Endpoint Groups](Network%20Endpoint%20Groups.md)
	- Configures [Pods](Pod.md) across clusters as backends
...
- Behaves similarly to regular [Ingress](Ingress.md)
	- Similar spec
